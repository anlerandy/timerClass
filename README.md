...
Return timer by `id`. Return `undefined` if no timer was found.  
...
Type: `objects`  
Options of `_.getById` are similar to those in `new Timer()` in addition with the two following.
...
Run the timer.  
If `callback` argument is a function, it returns `undefined`. If it's a `Promise`, it will return a `Promise`.
